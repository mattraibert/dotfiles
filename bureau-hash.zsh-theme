...
bureau_git_info () {
  ...
  hash=$(command git rev-parse --short HEAD 2> /dev/null) || return
  echo "${ref#refs/heads/}:${hash}"
}
...
bureau_node_version () {
  ver=$(command nodenv local 2> /dev/null) || return
  echo "$ZSH_THEME_NVM_PROMPT_PREFIX${ver}"
}
bureau_ruby_version () {
  ver=$(command rbenv local 2> /dev/null) || return
  echo "$ZSH_THEME_RUBY_PREFIX${ver}"
}
...
bureau_precmd () {
  right="$(bureau_node_version) $(bureau_ruby_version) $_1RIGHT"
